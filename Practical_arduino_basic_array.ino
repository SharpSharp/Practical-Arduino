/*  Arduino Test Track
    ...
    Use numberOfServos to setup the number you have connected.
...
int servoPins[numberOfServos] = {turnoutServoPin, signalServoPin};  // enter pin numbers. 1st number is for servos[0], 2nd for servos[1] etc
...
int servoTarget[numberOfServos] ;   // the angle you want the servo to turn to, will be set to either min or max
int servoPosition[numberOfServos] ; // current angle of servo

#define throwSpeed 30  // [ms] lower number is faster


#include<Servo.h>
/* assign servos in an array */
Servo allServos[numberOfServos];       // Define servo array

unsigned long throwDelay[numberOfServos];


bool turnoutClosed[numberOfServos];



void throwServo(int i) {
  throwDelay[i] = millis() + throwSpeed;
...
void setup() {
  ...
  for (int thisServo = 0; thisServo < numberOfServos; thisServo++) {
...
void loop() {
  ...
  if (digitalRead(throughSensorPin) == LOW && turnoutClosed) servoTarget[1] = servoMax[1];
  ...
  for (int thisServo = 0; thisServo < numberOfServos; thisServo++) {    // loop through all of the servos
